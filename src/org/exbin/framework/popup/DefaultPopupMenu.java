...
public class DefaultPopupMenu {
     ...
     * Registers default popup menu to AWT.
     */
    public static void registerOptional(boolean register) {
		if (popupEventQueue == null) {
			if (register) {
    	        DefaultPopupMenu defaultPopupMenu = getInstance();
	        	defaultPopupMenu.initDefaultPopupMenu();
    	        popupEventQueue = defaultPopupMenu.registerToEventQueue();
			}
		} else {
    ...
    private PopupEventQueue registerToEventQueue() {
    	PopupEventQueue queue = new PopupEventQueue();
